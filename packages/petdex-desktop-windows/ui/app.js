...
function syncManifestQuiet() {
  fetch("https://petdex.dev/api/manifest")
    ...
    .then(function (data) {
      ...
      // Warm the on-disk sprite cache for the first gallery page so the
      // first open renders instantly. Runs entirely in Rust (no IPC per
      // sprite) and only downloads sprites that aren't already cached.
      try {
        var invoke = window.__TAURI__.core.invoke;
        var firstPage = (allPets.length > 0 ? allPets : fresh)
          .slice(0, 24)
          .filter(function (p) { return !!p.sprite; })
          .map(function (p) { return p.sprite; });
        if (firstPage.length > 0) {
          invoke("precache_sprites", { urls: firstPage }).catch(function () {});
        }
      } catch (e) {}
    })
    .catch(function () {
...
var PAGE = 24;
var spriteCache = {}; // slug -> data URL, avoids re-fetching on re-render
...
// Preload sprites in parallel via the Rust disk cache (get_cached_sprite).
// On first view this downloads from the CDN (~25s per sprite from this
// network); on every subsequent view the bytes come from disk (~1ms),
// even across app restarts. Returns when all sprites are resolved.
function preloadSprites(pets) {
  ...
  var promises = pets.map(function (p) {
    ...
    return invoke("get_cached_sprite", { url: p.sprite })
      .then(function (b64) {
        // data URL — same shape that read_file_as_base64 uses elsewhere
        spriteCache[p.slug] = "data:image/webp;base64," + b64;
      })
        ...
        invoke("read_file_as_base64", { path: meta.sprite_path }).then(function (b64) {
          ...
          rmBtn.addEventListener("click", function (e) {
            ...
            invoke("uninstall_pet", { slug: p.slug })
              ...
              .catch(function (err) {
                // (previously referenced an out-of-scope `label` var, which
                // threw ReferenceError and swallowed the real error on every
                // uninstall failure — leaving the button stuck at "...")
                rmBtn.textContent = "Remove";
  ...
  setInterval(() => {
    invoke("read_cmd_file")
      .then((cmd) => {
        ...
        // SECURITY: each branch validates its inputs BEFORE invoking the
        // matching Rust command. The Rust side also validates, but we block
        // here too so a malicious cmd.json can't even reach the path-join.
        // The `eval` action was removed entirely — it ran arbitrary JS in the
        // WebView (which holds the full Tauri IPC surface), giving anything
        // that could drop a 30-byte file in ~/.petdex/runtime/ full RCE.
        var SAFE_SLUG = /^[a-z0-9][a-z0-9-]{0,63}$/;
        var SAFE_PANELS = { settings: 1, gallery: 1, switcher: 1 };

        if (cmd.action === "uninstall" && cmd.slug && SAFE_SLUG.test(cmd.slug)) {
        ...
        } else if (cmd.action === "switch" && cmd.slug && SAFE_SLUG.test(cmd.slug)) {
          invoke("set_active_pet", { slug: cmd.slug })
            .then(() => { setResult("CMD:SWITCH:OK"); loadActivePet(); })
            .catch((e) => setResult("CMD:SWITCH:ERR"));
        } else if (cmd.action === "openPanel" && cmd.panel && SAFE_PANELS[cmd.panel]) {
          openPanel(cmd.panel);
          setResult("CMD:PANEL:" + cmd.panel);
        } else {
          // Unknown or malformed command — log and ignore. Never surface the
          // raw cmd content in the label, it could be adversarial.
          setResult("CMD:UNKNOWN");
        }
      })
      .catch(() => {});
  }, 500);
