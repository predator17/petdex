...
type EditActionScope = "admin" | "collaborator";

export async function PendingEditsReviewPage({
  actionScope = "admin",
}: {
  actionScope?: EditActionScope;
}) {
  ...
  const rows =
    actionScope === "collaborator"
      ? allRows.filter((row) => !hasPendingAssetEdit(row))
      : allRows;
